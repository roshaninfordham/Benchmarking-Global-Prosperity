  ...
  return (
    <button onClick={flip} aria-label={label} aria-pressed={dark} className="theme-toggle" title={label}>
      <svg viewBox="0 0 24 24" width="20" height="20" aria-hidden>
        <circle className="tt-core" cx="12" cy="12" r={dark ? 5.5 : 4.2} fill="currentColor" />
        <g className="tt-rays" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" style={{ opacity: dark ? 0 : 1, transform: dark ? "rotate(45deg) scale(.4)" : "none", transformOrigin: "12px 12px", transition: "all .4s cubic-bezier(.3,1.4,.5,1)" }}>
