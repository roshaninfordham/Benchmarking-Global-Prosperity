// Builds public/map/world.json for the globe and flat map from Natural Earth 1:10m country shapes (variant set by SRC below).
// The source is 13 MB, so it is simplified to a light TopoJSON. Every country is treated the same way, and every piece of
// land is kept: pieces too small to simplify become points.
...
const KEEP = Number(process.argv[2] ?? 0.2); // share of points kept
...
const features = fc.features.map((f) => {
  ...
  // Pieces under about 160 km2 (the Maldives, Pacific atolls, most small islands) are too small to survive simplification,
