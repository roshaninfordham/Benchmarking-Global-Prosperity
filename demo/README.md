...
`bgp-demo.mp4` is a 1 minute 53 second walkthrough of the live platform (1080p, narrated, captions burned in). `bgp-demo.srt` has the same captions as a subtitle file, and `narration.md` has the script with times for slide notes.
...
3. `assemble.py` finds the white sync flash that `record.mjs` shows at a known instant, uses it to place each voice line exactly at its start time, normalises loudness to about -16 LUFS and encodes the MP4.
...
python subs.py         # rewrites narration.md and bgp-demo.srt
```
